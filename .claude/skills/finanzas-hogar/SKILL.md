...
- **Email inbound (BCI)**: correos del banco → webhook Postmark (`/api/webhooks/email/{householdId}?secret=`) → `parseBciEmail` → `pending_expense` + push. Al confirmar, se auto-vincula la tarjeta si hay exactamente UNA activa con esos últimos 4 dígitos.
